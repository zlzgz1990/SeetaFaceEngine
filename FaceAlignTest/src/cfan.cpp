...
void CCFAN::InitModel(const char *model_path)
{
  ...
  int pts_double_ = pts_num_ << 1;
  mean_shape_ = new float[pts_double_];
  fread(mean_shape_, sizeof(float), pts_double_, fp);
  for(int i = 0; i < pts_double_; i++) {
    mean_shape_[i]--;
  }
...
void CCFAN::FacialPointLocate(const unsigned char *gray_im, int im_width, int im_height, seeta::FaceInfo face_loc, float *facial_loc)
{
  ...
  memcpy(facial_loc, mean_shape_, pts_num_ * 2 * sizeof(float));
  ...
  memcpy(lan1_a[0], re_fea, fea_dim_ * sizeof(float));
  ...
  {
    facial_loc[i] += lan1_a[lan1_size_ - 1][i];
  }
  for (int i = 0; i < lan1_size_; i++)
  {
    delete[](lan1_a[i]);
  ...
  float x_scale = (float)lan2_resize_w / lan1_resize_w;
  float y_scale = (float)lan2_resize_h / lan1_resize_h;
  for (int i = 0; i < pts_num_; i++)
  {
    facial_loc[(i << 1)] *= x_scale;
    facial_loc[(i << 1) + 1] *= y_scale;
  }
  /*Extract the shape indexed SIFT features*/
...
void CCFAN::GetSubImg(const unsigned char *gray_im, int im_width, int im_height, float point_x, float point_y, int patch_size, BYTE *sub_img)
{
  ...
  for (int h = ty, ph = patch_top; h < by + 1; h++, ph++) {
    memcpy(sub_img + h * patch_size + lx, gray_im + ph * im_width + patch_left, rx - lx + 1);
  }
}
  ...
  unsigned char* dst_im, int dst_width, int dst_height)
{
  ...
  for (int n_y_d = 0; n_y_d < dst_height; n_y_d++) {
    for (int n_x_d = 0; n_x_d < dst_width; n_x_d++) {
      int lf_x_s = n_x_d * src_width / dst_width;
      int lf_y_s = n_y_d * src_height / dst_height;

      int n_x_s = lf_x_s <= (src_width - 2) ? lf_x_s : (src_width - 2);
      int n_y_s = lf_y_s <= (src_height - 2) ? lf_y_s : (src_height - 2);
